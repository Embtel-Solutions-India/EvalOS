...
| **Client portal legal pages** | COMPLETE (2026-09-25) | Public routes `/privacy`, `/disclaimer`, `/document-retention` (`client/src/pages/legal/`, content as JSX from the business's three policies; paths and the contact block in `constants/legal.ts`). `SiteFooter` — three one-sentence summaries, each linking its policy, plus links and address — under every screen: `PortalLayout`, a `PublicLayout` route over welcome/sign-in/set-password/sign-up, and `LegalPage`. In-context links: sign-up (agree to Privacy Policy, acknowledge Disclaimer), the document uploader (Retention), the send step (Disclaimer + Privacy). Checked in a browser 2026-09-25 | **The policies are the business's drafts and say they have not been reviewed by an attorney**; that internal note is not published, and attorney review before go-live is the business's call. Expert portal and client mail carry no legal links yet |
| **Case chat (Unit 57)** | PARTIAL — phase 1 of 3 (2026-09-26) | `V69` (conversations, trigger-guarded membership history, messages, reactions, read watermarks, push subscriptions); `chat/` — `ChatMembership` + `ChatRosterLoader`, `ConversationService`, `ChatAccess`, `MessageService` + `ChatInboxQuery`, `ChatLifecycleListener`, `CHAT_RECONCILE` (`ChatReconcileSweep`); `chat/live/` — `ChatRealtime` (Ably), `ChatChannels`, `ChatFanout`, `ChatPresence`, `ChatTyping`; `chat/push/` — `ChatPushNotifier`, `PushSender` (web-push 5.1.2, BouncyCastle 1.86, jose4j 0.9.6 — every chat dependency OSV-clean 2026-09-26; `PushLibraryCompatibilityTest` builds a real encrypted VAPID request offline); routes once in `web/ChatRoutes` on three surfaces; `CASE_MANAGER_REASSIGNED` event. Tests: `ChatMembershipTest`, `ChatRosterLoaderTest`, `ConversationServiceTest`, `ChatAccessTest`, `MessageServiceTest`, `ChatLifecycleListenerTest`, `ChatReconcileSweepTest`, `ChatChannelsTest`, `ChatRealtimeTest`, `ChatFanoutTest`, `ChatPresenceTest`, `ChatTypingTest`, `ChatPushNotifierTest`, `ChatApiPushTest`, `StaffChatControllerTest`, `PortalChatControllerTest`, 8 Postgres cases | **No UI yet** — staff app is phase 2, portals phase 3. `ABLY_API_KEY` and the VAPID keys must be set for live updates and push; **never exercised against a real Ably app** (the plan's manual check needs a key) |
