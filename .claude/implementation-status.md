...
| **GHL mirror (Unit 44)** | PARTIAL — slices A and D | `pipeline` + `pipeline_stage` (`V50`), `PipelineMirrorService`, `PipelineMirrorSweep` (`PIPELINE_MIRROR`, 1h), `PipelinePurpose`, `GET/PUT /api/ghl/pipelines`; `opportunity` (`V51`), `OpportunityMirrorService`, `client_application.opportunity_id` (`V53`), the correlation custom field on create; `ghl_opportunity_cache` DROPPED (`V52`) along with `CachedOpportunity`, `OpportunityCache` and `GhlOpportunityClient` | **44b** (`team_member_pipeline`, `PipelineScope.mine()` one id → a set, retires `intake-pipeline-name` — an authorisation change across Units 39/40) and **44c** (`contact`, merging `contact_snapshot` + `client_account`) are specced and unbuilt — `context/specs/44-ghl-tier1-mirror.md`. `OpportunityRepository.SCOPE` is `brandOnly` until 44b lines the pipeline axis up, so the pipeline scope lives in the finder signatures and `OpportunityRepositoryScopeTest` guards it. No sync engine: that is Unit 45 |
