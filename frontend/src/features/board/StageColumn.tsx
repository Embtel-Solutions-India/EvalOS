...
export default function StageColumn({
  ...
  emptyText?: string
  /** One line under the header, pinned with it — the opportunity board's stage value. */
  subtitle?: ReactNode
  /** Tints the header block and rules it off from the cards. The opportunity board's columns
   *  have no SLA rail to cap them, so without this the header reads as one more card. */
  banded?: boolean
  children: ReactNode
}) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`scroll-slim flex min-h-16 flex-col gap-2 overflow-y-auto px-2 pb-2 ${banded ? 'pt-2' : ''}`}
