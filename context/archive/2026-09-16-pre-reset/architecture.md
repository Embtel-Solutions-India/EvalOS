...
| GHL read API | `RestClient` against GHL's public API, `opportunities.readonly` (**inbound *pull*, Unit 24**) | The GM's marketing funnel view. **`GhlPipelineClient` is read-only and has no write method** — that is a property of this client, not of EvalOS: Unit 37 opened a write door beside it (`GhlWriteClient`, `GhlCalendarClient`), and invariant 2's read-only half died there. Two calls and a cached payload. The *aggregate* the screen draws is cached in `ghl_funnel_cache` (it was a heap map until 2026-08-26 — a per-process cache lost a completed background total on restart and could not hand one instance's result to another). **Unit 24 stored no opportunity rows**; Unit 38 added `ghl_opportunity_cache` (`V40`) for the desks, and it is a **cache, replaced wholesale**, never a record — a stage dragged five seconds ago would already be wrong in it. Both tables are caches, not records — safe to truncate, and not brand-scoped because the figures come from one global GHL location EvalOS cannot attribute to a brand. This is the third direction across the GHL seam — events in, events out, and now one pull — and it is the only one that is not a handoff |
...
`{brandId}/client/{contactSnapshotId}/…` is what a client sent us; `{brandId}/case/{caseId}/…` is what
...
**The client segment was the GHL contact id until 2026-09-14**, so one key resolved in both
systems with no mapping table. IE's sub-account swap on 2026-09-11 falsified that: every stored
GHL id names a contact that no longer exists, and a post-swap client has none at all, so the key
could not be built and `upload` threw. **A namespace a third party can revoke is not a namespace.**
Objects written under the old prefix **do not move** — reads resolve through the stored
`case_document.object_key`, which is authoritative, and only writes take the new shape. A bulk
re-key would be an S3 object copy, not a migration, and is not required for correctness.
...
**`{contactSnapshotId}` is EvalOS's own `contact_snapshot.id`** as of 2026-09-14. It was GHL's
contact id — the identifier `contact_snapshot.ghl_contact_id` holds and the Client Portal used —
on the reasoning that one client is one client in all three systems with **no mapping table
between them**. That reasoning was good and the sub-account swap falsified it (see above); the
identity story is unchanged, only the segment of the key. Email stays a **fallback** key
...
- **S3 document store (Unit 30)**: client documents and EvalOS's own artefacts. The case
  ...
  contact the case already points at (`client/{contact_snapshot.id}/`), so there is no second
...
| ~~Google Ads funnel~~ | — | **Unit 24's screen was REMOVED 2026-09-14.** The replacement GHL sub-account has no paid-search funnel, so the configured pipeline name matched nothing and the screen could only ever 502. Route, nav entry and `ads-pipeline-name` are gone from all three profiles; `MarketingPipelinePage` still serves two funnels, so reinstating it is a property, a route, a nav entry and an icon |
| Email marketing funnel | the whole thing, campaigns included | **nothing. Unit 26 *reads* it on the same terms** |
| Sales pipeline | **the record.** Every opportunity lives in GHL and nowhere else | **nothing.** Unit 27 *reads* it as a GM funnel; Units 36–41 gave Sales and Marketing desks that read and **write** it. EvalOS still holds no opportunity of its own — `ghl_opportunity_cache` (`V40`) is a droppable cache, and Unit 44 is where an owned mirror arrives |

Those rows are the exception the table needed, and the distinction they draw
is the one to keep: **custody and visibility are different things.**

**Visibility stopped implying read-only in the `00b` programme, and that is the thing to
read carefully.** Unit 29 first let a sales executive move a stage from an EvalOS screen,
was removed, and the removal restored a read-only `GhlHttp` — then **Units 36–41 reversed
that deliberately and in writing**: invariant 2 died at Unit 37, `GhlHttp` has write verbs
again behind `GhlWriteClient`, and Sales and Marketing now work in EvalOS rather than in
GHL. **Custody did not move with it.** GHL still owns the opportunity, the pipeline, the
stage and the invoice; a case is still born only of a won opportunity through Handoff A.

**What EvalOS holds of that world, and on what terms:**

- `ghl_opportunity_cache` (`V40`) — every column a field GHL owns, **droppable without
  loss**, replaced wholesale rather than upserted.
- `opportunity_note` (`V41`) — **owned outright.** GHL has nowhere to put it: its note
  endpoints hang off the *contact*, so a repeat client's two deals would share one stream.
- `meeting` (`V47`) and `follow_up` (`V48`, 2026-09-14) — **mirrors of GHL's own booking
  response, never authorities.** They exist because the desk was otherwise write-only: a
  meeting booked from EvalOS could never be shown again by EvalOS. `status` is GHL's word,
  stored verbatim, and neither table may grow a lifecycle of its own.

The old rule — *no EvalOS table copies a pipeline row* — has become a narrower and more
useful one: **a copy is allowed when it is droppable or reconcilable, and never when it
would become a second answer.** A stage a salesperson dragged five seconds ago is still
wrong in a copy, which is why the cache is replaced wholesale and why `00c`'s owned mirror
(Unit 44) needs a sync engine before it needs a table.
...
1. **Brand isolation.** Every scoped query filters by `brand_id`; no code path
   ...
   **One stated exception, and it is not a query over EvalOS rows**: the GHL pipeline
   reads (Units 24, 26 and 27, and Unit 51's `GET /api/metrics/gm`) go to the one GHL
   sub-account named by `evalos.ghl.location-id`, a *global* setting with no link to a
   brand — so no `brand_id` predicate exists that could narrow them. **GM-only** follows
   from that.

   **Unit 51's route is the one place the exception meets a `brandId` parameter, and it
   does not weaken it.** `/api/metrics/gm` accepts `brandId` because **half its payload is
   EvalOS rows** — the case-by-service and evaluation figures, which are scoped like every
   other case read and narrow correctly. The GHL half ignores it, because there is nothing
   to narrow. That asymmetry is safe only while the gate is `hasRole('GM')`:
   `GmOverviewRouteTest` pins every other role, the Brand Manager included, and the
   `RoleDashboard` table sends the Brand Manager to the brand-scoped `RevenueDashboard`
   instead.
   ...
   **unattributable**, not on it spanning brands — and the Brand Manager is excluded
   ...
   Unit 25a then re-scopes the remaining funnel screens together (two since the Google Ads screen was removed on 2026-09-14).
