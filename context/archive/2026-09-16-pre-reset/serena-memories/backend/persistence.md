...
**`V49` adds `client_application` (Unit 43) — the client's request, before it is a case.**
`client_account_id` not `ghl_contact_id` (the account survives the CRM being replaced);
`service_id` is text and not an enum, because the catalog is frontend data and a column migrated
on every rename would make the catalog's point false; `service_name` is denormalised as the name
**at the moment of choosing**, so a catalog edit cannot change what a client is recorded as having
asked for. `ghl_opportunity_id` is **not** an FK — the opportunity is GHL's, and
`ghl_opportunity_cache` is droppable. Three index decisions worth keeping:
`client_application_one_draft_idx` is a **partial unique index on the account where status =
'DRAFT'**, which is the only thing containing `createOpportunity`'s duplicate risk; the submitted
-at CHECK is biconditional with **the NULL arm written explicitly**, because a CHECK evaluating to
NULL passes in Postgres (V39's trap). `answers` is jsonb holding an **array** of
`{id, label, value}` — see `mem:backend/core` for why the label travels with the answer.

**`V47` adds `meeting` and `V48` adds `follow_up` (2026-09-14) — mirrors of GHL's booking
response, not caches and not authorities.** They exist because the sales desk was **write-only**:
`SalesMeetingService` handed the booking response to the caller and kept nothing, so no EvalOS
screen could show a meeting it had itself created. Shape rules, both tables: `ghl_appointment_id`
/ `ghl_task_id` **unique per brand, not globally** (two brands will one day hold two GHL
locations, and ids are unique only within one); `ghl_opportunity_id` is **text, not an FK** —
the opportunity lives in GHL and EvalOS has no opportunity table until Unit 44 — and is indexed
so a deal's meetings are one lookup; `status` is **GHL's word verbatim**, never an enum and never
a lifecycle; everything is `updatable = false` except what a reschedule moves and the sync stamp,
so a mis-wired update fails loudly instead of silently re-pointing a row. `ghl_updated_at` /
`synced_at` are there for Unit 45's reconciling sweep. `follow_up.completed` is **a convenience,
not an authority**: the desk's "mark done" writes to GHL first and here second, so the common case
is right immediately, and a task completed in GHL itself waits for the sweep.
