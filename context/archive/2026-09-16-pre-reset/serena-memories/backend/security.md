...
> - **⚠ Both of the next two lines were WRONG and are corrected here rather than beside.**
>   The first said *"a separate Client Portal application writes client uploads; EvalOS's
>   credential is read-only on `client/`"* — spec 30's first draft, corrected the same day. **The
>   portal is a separate FRONTEND with no AWS credential at all**; it calls EvalOS, and **EvalOS is
>   the only writer**, under both prefixes. Do not revive the split.
> - `{clientId}` was **GHL's contact id** — one client across GHL, the portal and EvalOS with no
>   mapping table — and is **`contact_snapshot.id` since 2026-09-14**. The sub-account swap of
>   2026-09-11 turned every stored GHL id into a name for a contact that does not exist, and left
>   post-swap clients with none, so the key could not be built and `upload` threw: **a namespace a
>   third party can revoke is not a namespace.** Objects under the old prefix do not move —
>   `case_document.object_key` is authoritative on read. Unit 43's funnel keys on
>   `client_account.id` instead, because it uploads before a case exists; the two converge at
>   Unit 44. **Email stays a fallback key (V27), never the identity.**
...
second kind of session. The only config change is `permitAll` on the five auth routes, and those
routes stay behind the same per-IP limiter.

**Those five are named one by one and by method** — `POST` on `identify`, **`sign-up`**,
`sign-in`, `forgot-password`, `set-password` — **never `/api/portal/auth/**`.** A wildcard opens
the next route added under that prefix the moment it is written, silently; the list makes it
arrive as a 401 in that route's own test instead.

**`sign-up` (2026-09-15) is how a `client_account` comes into existence, and until it nothing
did.** Every row came from `V45`, a one-shot backfill — `new ClientAccount(...)` appeared nowhere
in `src/main` — so Handoff A wrote a case and a `contact_snapshot` and no account, and **every
client acquired after 2026-09-12 was permanently locked out of the portal** with `identify`
answering `UNKNOWN` for ever. Three properties to keep:
- **`GhlWriteClient.upsertContact` resolves create-or-reuse**, so EvalOS never decides whether a
  contact is new (invariant 7) and one client never becomes two contacts.
- **It returns an `IdentifyState`, never a token.** The address may be one GHL already holds,
  whose cases sit behind it, so a session for an unproven mailbox is takeover by typing a
  stranger's email. The emailed set-password link is the only proof, as it is for `V45`'s clients.
- **An address already held creates nothing and calls GHL not at all**; a GHL outage refuses with
  502 rather than writing a local-only account no salesperson can see; a raced duplicate is
  caught on `client_account_brand_email_key` and answered, not 500.
    ...
    **new `Tier.PIPELINE`**, keyed on `team_member.ghl_pipeline_id`.
    ...
    - **Single-brand ceiling, enforced:** `evalos.ghl.sales-brand`. A pipeline-scoped member of
      ...
      **The named brand is International Evaluations** — `application-local.yml` defaults it to
      ...
      (**`WY6bW2xUCI8Tz8gw7aLJ` since 2026-09-14** — the local default was repointed off the
      abandoned `kBumF0uUOmMBB5bneYjx`). It was blank until then, which meant the desks Units
      36-41 built were unreachable on every laptop: built, tested, and impossible to log into.
      `db/seed-local/V908` seeds the IE logins; **`V909` repoints them (2026-09-14)** — a new
      file, never an edit, because the local profile relaxes `*:missing` and not `*:checksum`.
      The live account splits sales **by service line, not by client segment** (Evaluation &
      Translation / PERM + business plan / Expert Opinion + RFE), and marketing has three BDE
      pipelines, so V909 adds a third MARKETING login. `segment` did not move with the names:
      `00b` §1.1 makes it display-and-reporting only and nothing branches on it.
      **`ghl_pipeline_id` is an ACCESS KEY, and a wrong one is invisible** — GHL returns nothing
      for a pipeline it does not have, so the desk draws zero columns with no error. An empty
      board and a dead key look identical. Live desks are repointed through
      `PUT /api/team-members/{id}/ghl-pipeline` as the GM (cutover item C4), which **has no UI
      and is a curl**. Deployed environments must still set `GHL_SALES_BRAND_ID`; the default is
      local only.
