...
> - **⚠ Both of the next two lines were WRONG and are corrected here rather than beside.** The
>   first said *"a separate Client Portal application writes client uploads; EvalOS's credential is
>   read-only on `client/`"* — spec 30's first draft, corrected the same day. **The portal is a
>   separate FRONTEND with no AWS credential at all**; it calls EvalOS, and **EvalOS is the only
>   writer**, under both prefixes. Do not revive the split.
> - `{clientId}` was **GHL's contact id** and is **`contact_snapshot.id` since 2026-09-14**: the
>   sub-account swap turned every stored GHL id into a name for a contact that does not exist, so
>   the key could not be built and `upload` threw. **A namespace a third party can revoke is not a
>   namespace.** Objects under the old prefix do not move — `case_document.object_key` is
>   authoritative on read. **Email stays a fallback key (V27), never the identity.**
...
- **Built — 01–12** (config + envelope, the tenancy/auth/RBAC spine, the domain schema, the case
  ...
  **05b** (`opportunity.won` intake, which **superseded 05a** — 05a's "contact created, not payment
  ...
  (five `*MetricsService` behind seven `MetricsController` routes — eight since Unit 51's `/gm` — but see below: **17a's gap list
...
**GHL's `date`/`endDate` filter on the opportunity's `createdAt`, and nothing filters on the
status-change date.** Verified against the live API 2026-09-15. That is the fact the GM overview
(Unit 51) is shaped around: *"won this month"* is **not a query**, because a deal opened in July
and won in September is a September win and a July creation. `GhlPipelineClient.opportunitiesIn`
therefore has a four-arg overload taking a GHL `status`, and `GmOverviewService` reads
`status=won` over `evalos.sales.won-lookback-days` (180) of *creations* and buckets them by
`lastStatusChangeAt`. **That property is a correctness bound, not a tuning knob** — set it shorter
than the real sales cycle and the won figure is silently low. `status=won` is what keeps the read
small: six months of wins is a fraction of six months of leads. Unit 44's mirror removes the
problem rather than caching it.

`GhlPipelineClient.Opportunity` is now ten fields, not three (`id`, `pipelineId`,
...
carries the contact's name, email, phone, tags, attributions and custom fields on every row, and
binding only these keeps marketing PII out of an EvalOS response by construction. **`assignedTo`
is read by nothing**: EvalOS has no mapping from a GHL user to a `team_member`, so every
per-salesperson figure keys on `team_member.ghl_pipeline_id`, the link Unit 36 actually built.

**`GET /api/metrics/gm` is `hasRole('GM')` and nothing wider** (`GmOverviewRouteTest` pins every
other role, Brand Manager included). Half of it reads the unattributable location, and invariant
1's exception holds only for the cross-brand reader. It **answers 200 when GHL is down**: the
production half is EvalOS's own rows and stays true, and `pipelineUnavailable` names the failure
rather than taking four working tiles down with the four that broke. **`GmOverviewService` has no
cache** (marked `ponytail:`) — two searches per desk, ~8 paced requests for one reader; the
upgrade path is a `"GM"` row in the existing `ghl_funnel_cache`, which needs no migration.
...
`MarketingController` (`GET /api/marketing/email-pipeline` and `/sales-pipeline`, both
`hasRole('GM')`; **`/ads-pipeline` was removed on 2026-09-14** — the new GHL sub-account has no
paid-search funnel, so the configured name matched nothing and the route could only ever 502, each taking `range` plus `from`/`to` for `range=custom`) is **its own controller rather than a sixth route on `MetricsController`** —
...
## Unit 43 — the client asks for a service

`client_application` (`V49`) is **not a case and not a copy of a GHL opportunity.** A case is born
only of a won opportunity through Handoff A (invariant 8); this row carries the questionnaire
between a client filling it in and a salesperson pricing it, which is a stretch of the client's
journey EvalOS had no record of at all.

- **The opportunity is created when the client picks a service**, not at submit — a lead who stops
  mid-questionnaire (the longest part of the funnel) must already be on a board.
- **EvalOS sends NO stage and NO assignee, and this is a boundary rather than an omission.** It
  puts the opportunity on `evalos.ghl.intake-pipeline-name` and stops; placement, routing and
  ownership are **GHL's automation's**, which is what `00b` keeps GHL for. A `hot-stage-name`
  property existed for one afternoon on 2026-09-15 and was removed on the business's instruction —
  it made EvalOS hold a second opinion about pipeline placement, stale the first time anyone
  reworked the workflow in GHL. **Do not reintroduce it**, and do not send `assignedTo` either:
  no column links a GHL user to an EvalOS `team_member`.
  **`createOpportunity`, never `upsertOpportunity`**: a repeat client's second evaluation is a
  genuine second deal, and upsert would overwrite the first. The duplicate risk that reintroduces
  is contained by `client_application_one_draft_idx`, a partial unique index — one unfinished
  request per client.
- **The row is written before GHL is called**, and a GHL outage is *swallowed* on start/save
  (retried next write) and *refused* on submit (502). An application Sales cannot see reads to the
  client as "sent" and to the business as nothing.
- **`answers` is a jsonb ARRAY of `{id, label, value}`**, never a map: the question text lives in
  the client portal's catalog and the staff app is a separate build that cannot import it.
- **The catalog is frontend data and is NOT mirrored in Java.** Submit validates shape and size,
  not completeness — a marked `ponytail:` ceiling. If completeness ever has to be enforced
  server-side, move the catalog to the server and serve it to the SPA; do not write a second copy.
- `ApplicationReviewController` (`GET /api/opportunities/{id}/application`) answers **200 with a
  null payload** for a deal that did not come from the portal, which is most of the board.
  **Sales AND Production**, brand-scoped and deliberately *not* pipeline-scoped — `requireMine`
  would refuse Production, who hold no pipeline. Its own controller, because `SalesDeskController`
  is `hasRole('SALES')` throughout.

## GHL clients — six, and the split is deliberate

`GhlHttp` is the transport door (Unit 37: a closed verb list, no retry on writes, no audit —
it cannot know what a write *means*). Six clients sit on it:

- **`GhlPipelineClient`** — funnel aggregates for the GM screens. Matches pipelines by
  NAME, so a rename breaks a dashboard loudly.
- **`GhlOpportunityClient`** — board reads, feeding `ghl_opportunity_cache`.
- **`GhlWriteClient`** — contacts, opportunities, follow-up tasks. Creates go through **upsert**,
  never POST: GHL demands idempotency and offers no key, and upsert is the only thing that makes a
  double-submit safe. **One exception since 2026-09-14** — `createDeal`, behind
  `SalesOpportunityController`, takes `POST /opportunities/` and **brings the duplicate risk back
  knowingly**, because upsert is right for a lead and wrong for a repeat client buying a second
  service. `DuplicateDealException` → 409 is what the salesperson sees instead.
- **`GhlCalendarClient`** (Unit 40's meetings, 2026-09-11; extended 2026-09-14) — calendars, free
  slots, book, reschedule, appointment notes.
- **`GhlUserClient`** (2026-09-14) — the location's users, for the booking dialog's team-member
  picker. **Names and emails only**: carrying a colleague's `roles`, `scopes` and `permissions`
  into EvalOS would be holding a second copy of GHL's access model. Deleted users stay in GHL's
  list and must not reach the picker.
- **`GhlCustomFieldClient`** (2026-09-14) — the location's opportunity custom fields, for the new
  deal form. Matched on **`fieldKey`**, never on id (location-scoped, dies on the next sub-account
  swap) or label (renameable in the GHL UI).
...
meetings and there is nothing behind the call to prevent it.

> **The local mirror the truth model used to refuse now exists (2026-09-14): `meeting` (`V47`) and
> `follow_up` (`V48`).** The refusal was coherent while a meeting was a fire-and-forget write, and
> it is what made the sales desk **write-only** — a salesperson booked Thursday's call and no
> EvalOS screen could ever show it again, which defeats the stated purpose of Units 36-41 ("so
> that they never open GHL"). Both tables mirror **GHL's own booking response**, which
> `GhlCalendarClient.AppointmentRow` already parses and `GhlCalendarClientHttpTest` already
> covers. Three read-back routes were compared and rejected: `GET /calendars/events` needs a scope
> that is not granted; `GET /contacts/{id}/appointments` is N calls per screen; and
> `GET /opportunities/search?getCalendarEvents` is the right long-term read and **stays the plan**,
> but the joined field is empty across all 100 rows of the busiest pipeline, so a binding written
> against it today would be a guess that compiles. This is also `00c`'s direction: EvalOS holds
> its own rows and syncs them, and a meeting booked here survives the sync being switched off.
> **Neither table is the authority.** GHL owns appointments and tasks; one moved in GHL is not
> reflected until Unit 45's sweep reconciles, which `ghl_updated_at` / `synced_at` exist to let it
> compare. `status` is GHL's own word stored verbatim as a `String` — **not an enum**, because a
> remote vocabulary can gain a value tomorrow and an enum turns that into a deserialization
> failure on a screen that only wanted to print it. **These tables must not grow a lifecycle.**
...
- **`evalos.ghl.*` (Unit 24).** `token` (`GHL_API_TOKEN`) and `location-id` (`GHL_LOCATION_ID`)
  ...
  **one read-only GM screen**, so the app serves everything else and answers 502 there.
  ...
  has a default and a bump is an environment change), `email-pipeline-name`
  (`Shivangi's Email Marketing`, Unit 26), `sales-pipeline-name`
  (`Aditya's pipeline`, Unit 27 — **written with ONE space where GHL stores TWO; the client
  normalises whitespace, so do not "fix" it**) — two names since `ads-pipeline-name` left all
  three profiles on 2026-09-14, and **not** a list: a list
