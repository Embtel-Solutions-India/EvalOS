...
> ## 🔴 AUDIT 2026-09-13 — read `context/specs/00d-platform-audit-and-alignment.md` first
>
> Six independent senior audits read the code (not the docs). Full reports:
> `context/audit/2026-09-13/`. The build is **green** — 966 tests, 0 failures — and the audit
> found no IDOR, no unscoped read that is a bug, no mock-backed routed screen, and no residue
> from the removed units. What it found instead, none of it recorded anywhere else:
>
> - **Handoff A is dead.** The `opportunity.won` workflow lived in the GHL sub-account
>   abandoned 2026-09-11 and was never recreated. **No case is created by anything.** Also:
>   `team_member.ghl_pipeline_id` holds dead ids, so every Sales/Marketing desk draws zero
>   columns **with no error**; `application-local.yml:112` still defaults to the dead location.
> - ~~**`PortalCaseService.upload` is broken for every token Unit 42 mints**~~ — **FIXED
>   2026-09-14.** It called `cases.findById(principal.caseId())` instead of
>   `authorized(principal)`, and a party-scoped principal's `caseId` is null *by definition*; it
>   also required a live `ghl_contact_id` for the S3 key, which no post-cutover client has. Both
>   closed, with the party-scoped test that would have caught them. **Still open: the per-case
>   variant** — two or more cases refuses and needs `00d` §2b item 3's routes and a picker.
> - **SALES matching nothing is structural, not a missing grant.** `Case` has no pipeline
>   column, so `ScopePredicate`'s PIPELINE arm returns `cb.disjunction()`. Do **not** fix the
>   status loop by widening a tier — add `PortalStageProjection.forSales` beside `forClient`
>   and `forExpert`, and join `Case.ghl_opportunity_id` to `OpportunityNote.ghl_opportunity_id`
>   (same value, same database, unjoined since 2026-09-11).
> - **`PipelineScope` authorises every Sales/Marketing write against the droppable
>   `ghl_opportunity_cache`.** Access control has a TTL. `V40`'s "no write path reads it to
>   decide anything" is false today.
> - **`00c` needs six amendments before Unit 44 starts** (00d §6). Load-bearing: at-least-once
>   outbox delivery over a non-idempotent `upsert` create makes duplicates — needs **one tier-2
>   custom field at 44, not 47**. Also: per-field ownership, not blanket "EvalOS wins" (a
>   blanket rule reverts the GHL automations `00b` kept GHL for).
> - **ENM is the largest unscheduled gap** — a roster and a dashboard, no candidate entity, no
>   stages, no assignee axis. Unit 22 slice 4 **refused** the pipeline in writing; Unit 50 must
>   reverse that in a spec before any code.
>
> Also live: `upsertOpportunity` sends no `id`, so a repeat client's second deal in the same
> pipeline **overwrites the first** — but `39` §3a decided that knowingly and named
> `POST /opportunities/` as the escape hatch that *"brings the duplicate risk back, knowingly"*.
> **Do not switch the verb**; it breaks `43` §7's retry model. Fix belongs at Unit 44 with the
> correlation key.
>
> **Four traps the audit itself fell into — do not repeat them:**
> - **`DomainInvariantsTest` permits exactly ONE injector of `CaseIntakeService`**
>   (`GhlOpportunityHandler`). A `contact.created` handler calling `syncContact` **fails the
>   build**. Extract a `ContactSnapshotService` first.
> - **Do not add a `GET /opportunities/{id}` fall-through to `PipelineScope`** — `39` §7 rejected
>   it on the rate budget. Cache-on-write closes the live half for free.
> - **Unit 25 is not buildable first**: it grants `opportunities.readonly` only, **deletes** the
>   PIT six live features now need, and waits on a GHL Marketplace app nobody has created. Take
>   `brand.ghl_location_id` + per-brand `GhlHttp` instead and re-spec 25.
> - ~~**Unit 43 already ships `pipeline`/`pipeline_stage`**~~ — **it does not, and this line was
>   wrong before 43 was built and is wrong after.** 43 shipped 2026-09-15 without those tables:
>   `GhlPipelineClient.pipelineNamed` answers "which stage is hot" live behind its five-minute
>   cache, so nothing needed the mirror. **Unit 44 still owes it.** 43 *did* decide portal signup
>   identity and verbatim stage ids — read it before designing either again. **The document key
>   is `contact_snapshot.id` for the case path** (`DocumentStore.clientKey`, 2026-09-14) and
>   `client_account.id` for the funnel path (`43` §5); two EvalOS-owned prefixes, converging at
>   Unit 44.
>
> "Writes do not retry" is refused in **three** specs — `37` §7, `39` §3a, `43` §7. The outbox
> amends all three; that is an edit to each, not a side effect of a sweep.
>
> **Facts worth not re-deriving:**
> - The API is **129 routes across 28 controllers** (annotation count), not the "105 across 27"
>   the backend report's summary line said. Per-route and per-screen decisions:
>   `context/specs/00d-appendix-screen-and-api-decisions.md` — 23 routes (18%) and 14 of 39
>   screens (36%) carry a non-KEEP decision.
> - **Questionnaires do not exist in the backend.** No entity, table, route or service; the
>   conditional engine was deleted in `f9f1165`. `43` restores the client/Sales/Production halves;
>   **the expert seeing answers is in no spec** (`00d` §12 g2).
> - **`PUT /api/team-members/{id}/ghl-pipeline` has zero UI callers**, so reassigning a pipeline
>   — which the cutover requires — is a curl or a DB write today.
> - The largest remaining work category is **screens over endpoints that already exist and are
>   already tested**: the expert's case list and payouts, the Sales case-status read, team
>   administration, the sync-status panel.
...
> - **Keys are brand-first**: `{brandId}/client/{clientId}/{documentId}` and
...
> - `{clientId}` is **EvalOS's own `contact_snapshot.id`** as of 2026-09-14. It used to be GHL's
>   contact id, so that one key resolved in both systems with no mapping table — a real benefit,
>   falsified by the 2026-09-11 sub-account swap: every stored GHL id names a contact that no
>   longer exists, and a post-swap client has none at all, so the key could not be built and
>   `upload` threw. **A namespace a third party can revoke is not a namespace.** Objects written
>   under the old prefix **do not move**: reads resolve through the stored
>   `case_document.object_key`, which is authoritative; only writes take the new shape.
>   **Email stays a fallback key (V27), never the identity, and never in a key.**
...
other side of it.** EvalOS *reads* GHL pipelines onto GM screens — **Shivangi's Email Marketing**
(Unit 26) and **Aditya's pipeline**, the sales team's own funnel (Unit 27) — in the one configured
location, and owns none of them:

> **The Google ADS Pipeline screen (Unit 24) was REMOVED on 2026-09-14.** The GHL sub-account IE
> moved to on 2026-09-11 has no paid-search funnel — its pipelines are BDE-1/2/3 (marketing),
> Sales-1..4 by service line, and ENM — so `evalos.ghl.ads-pipeline-name` matched nothing and the
> screen would have answered 502 forever. **A screen that cannot succeed is worse than an absent
> one.** Removed rather than repointed because nobody named a replacement funnel. The route
> (`/marketing/google-ads`), the nav entry and the property (all three profiles) are gone;
> `MarketingPipelinePage`, `GhlPipelineClient` and `MarketingController`'s two other routes are
> untouched, so reinstating it is a property, a route, a nav entry and an icon. Unit 24 remains the
> unit that introduced the pull direction, and everything below about *how* it reads still holds. no lead created, no
...
> **CURRENT STATE, 2026-09-15 — read this before the historical paragraphs below, which are kept
> as the record of how each unit landed and whose test counts and migration numbers are stale by
> design.** Units 01–43 are behind us; the live programme is `00d`'s **Phase 0**, which is repair
> work with **no spec** (`00d-implementation-runbook.md` §2 — the ordering *is* the content).
> Green as of now: `./mvnw test` **974 tests, 0 failures**; `frontend` 127 vitest tests; all three
> frontends build. Migrations run to **`V49`**.
>
> **Phase 0 done (2026-09-14/15):** the local `location-id` default repointed off the abandoned
> sub-account; `V909` repointing the seeded desks; the Google Ads funnel removed by decision;
> `PortalCaseService.upload` authorizing through `authorized(principal)` instead of
> `findById(principal.caseId())` (which was `null` by construction for every Unit 42 token, so
> every client upload 500'd); the client document key re-based on `contact_snapshot.id`;
> `BoardView`'s `reloads`-in-the-deps bug.
> **Phase 0 still open:** the `opportunity.won` workflow in the new sub-account — **Handoff A is
> dead until it exists, so nothing creates a case** — the new PIT, cutover item C4 on the live
> desks (a curl; the route has no UI), `TRUNCATE ghl_opportunity_cache`, *Allow Duplicate Contact*,
> the three per-case document routes plus the client `CasePicker`, `WebhookReplaySweep`,
> `GET /api/admin/ghl/status`, two more interaction defects, and four stale `unavailable` cards.
>
> **Unit 43 shipped 2026-09-15 and the client flow is real end to end** — sign up → sign in →
> dashboard → request a service → questionnaire → review → send → track, with Sales reading the
> answers on the deal card. `V49__client_application.sql`; the opportunity is created when the
> client **picks a service**, not at submit, **with no stage and no assignee** — GHL's automation
> places the deal, and a `hot-stage-name` property was removed the same day for holding a second
> opinion about that. Three things the spec asked for and this did not do,
> each recorded in `43`: no funnel document upload, no `OpportunityNote` copy of the answers, and
> no `pipeline`/`pipeline_stage` mirror. See `mem:backend/core`.
>
> **Alongside it, the Sales desk gained the half it never had (2026-09-14):** opening a deal
> (`SalesOpportunityController` + `NewDealForm`), meetings and follow-ups as **local mirrors**
> (`V47`, `V48`), a diary at `/meetings`, and a `PipelineDashboard` for SALES and MARKETING. See
> `mem:backend/core`, `mem:backend/persistence`, `mem:frontend/core`.
...
- **S3 is the document store; EvalOS still hosts no bytes. It gained SMTP in Unit 42, for
  ...
  **Google Drive is gone as of Unit 30** — API client, service account, dependency and the
  ...
  Keys are **brand-first**: `{brandId}/client/{contactSnapshotId}/{documentId}` (an EvalOS UUID
  since 2026-09-14, not the GHL contact id — see the DocumentStore block above) and
  `{brandId}/case/{caseId}/{folder}/{documentId}` — the object name is the document's own id,
