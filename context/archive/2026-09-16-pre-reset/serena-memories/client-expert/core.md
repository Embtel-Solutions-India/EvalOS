...
> helper.**
>
> **`/signup` is real as of 2026-09-15, and `Start.tsx` is deleted.** That placeholder said we
> could not take a new client at all — accurate, because **nothing created a `client_account` at
> runtime**. `SignUp.tsx` posts to `/auth/sign-up`, which creates the account and the GHL contact
> and answers an `IdentifyState`: **it never signs anyone in**, because the address may be one GHL
> already holds. `/start` redirects for one release. **It creates an account, not an evaluation** —
> choosing a service and answering the questionnaire are Unit 43, reached from the dashboard.
>
> **`/requests/new` is Unit 43's funnel (2026-09-15), and it is ONE route with three steps** —
> service, questionnaire, review — not the seven the deleted version had. Those seven existed
> because the funnel was a public pre-account journey where each step needed a URL to survive a
> sign-up; the client is signed in throughout now, and resumption is a `client_application` row
> rather than `localStorage`. **The catalog, question groups and conditional engine are RECOVERED
> from `f9f1165^` unchanged** and need no new dependency; `AboutYou`, `intakeService` and the mock
> `ClientRequest`/`RequestStatus` types did not come back.
>
> **Answers are sent as an array of `{id, label, value}`**, not a map — the staff app is a separate
> build with no catalog, so the question travels with the answer.
>
> **`/requests` shows two lists and must not merge them**: a *request* is what the client sent and
> Sales has not priced, a *case* is work in progress. One list would need this app to invent a
> status vocabulary spanning both, which is what D5 forbids.
>
> **A client with no cases now sees an EMPTY documents screen, not a refusal.** `documents` used
> to route through `authorized`, which answers a party token with no case by throwing
> `AmbiguousCaseException`; self-signup made that the ordinary first minute of an account's life.
> Two or more cases still refuses and still wants a case picker.
