...
**Every role reads live figures as of Unit 22; the placeholder tiles are gone.**

**The GM and Brand Manager stopped sharing `RevenueDashboard` on 2026-09-15 (Unit 51).** They
shared it on the argument that the difference between them was a filter — the GM's payload carried
a per-brand breakdown, the Brand Manager's was their brand. That held while every figure on it was
brand-scoped. The GM's new `GmDashboard` also reads the **GHL location**, which is invariant 1's
one stated exception and is licensed *only* while the reader is the cross-brand role: a Brand
Manager shown a pipeline figure cannot tell, and neither can the server, whether it is theirs. So
**GM → `GmDashboard`, BRAND_MANAGER → `RevenueDashboard`**, and widening `/api/metrics/gm` past
`hasRole('GM')` voids that exception's own argument (`GmOverviewRouteTest` fails if it is).

They were never the same role with a filter in any case: `SEES_STRATEGY_NOTES` excludes the Brand
Manager, so nothing on a shared screen may assume oversight means full visibility.

**`GmDashboard` composes five reads and computes nothing twice.** `/metrics/gm` is the only new
one; `/metrics/revenue`, `/metrics/pm`, `/metrics/expert-network` and `/opportunities/board` were
already reachable by this role, and every figure taken from them is *taken*, not recomputed. Each
read carries its own `CardState`, so GHL being down cannot blank the evaluation department. The
`/metrics/gm` payload's `pipelineUnavailable` is rendered as an `error` state on exactly the tiles
it invalidates — a 200 carrying it is a failed read the server chose not to fail the whole request
over, and rendering it as `ok` would print `$0` won for a month that may have gone well.
`STALE_DAYS` and `staleDeals` live in `dashboards/dealAge.ts`, not in either screen that reads
them: two dashboards report on that threshold and a copy in both drifts the first time one is
tuned. (It also keeps `PipelineDashboard` a file that exports only components, which is what fast
refresh needs — the warning that moved them.)
...
**`/marketing/email` and `/sales/pipeline` are GM-only, and that is a
scoping fact rather than a taste call** (Units 26, 27). `NavGroup` gained **`Marketing`** and
then **`Sales`**, both sitting above `Pipeline` because these are the funnels *before* EvalOS takes
custody and Pipeline is everything after. The role list is `['GM']` because both endpoints read
...
and these are the figures that cannot honour it. `navigation.test.ts` loops the assertion over
every such path with the reasoning inline, so a screen added without the same door fails a test
rather than shipping.

> **`/marketing/google-ads` (Unit 24) was REMOVED on 2026-09-14**, route, nav entry and config
> property. The GHL sub-account IE moved to on 2026-09-11 has no paid-search funnel, so the screen
> could only ever answer 502 — and a screen that cannot succeed is worse than an absent one.
> `MarketingPipelinePage` still serves two funnels, so reinstating it is a property, a route, a
> nav entry and an icon. Do not re-add it without a named replacement funnel.
...
**SALES and MARKETING got a dashboard on 2026-09-14 (`PipelineDashboard`), and the argument
against one was narrowed rather than thrown away.** It ran: every other role lands on a summary of
work that lives in EvalOS, and these two work a GHL pipeline, so **their board is their
dashboard**. That still holds for anything derived by **counting cards** — a tile reading "14 open
deals" is the board's own column totals in a worse place. It does **not** hold for **age**: a
Kanban groups by stage in GHL's order and never shows how long a deal has sat there, so the
dashboard is the deals-by-age read and not a scoreboard. It reads GHL opportunities and **no case
at all**, which is precisely why the two roles join the **`/dashboard` nav entry and no other**:
`PRODUCTION_ROLES` earned its name by excluding them from every case-reading screen, so the entry
spells out `[...PRODUCTION_ROLES, 'SALES', 'MARKETING']` rather than widening the constant.

**`/meetings` (2026-09-14) is the salesperson's diary**, and the one place a meeting can be booked
without first finding the deal on the board. Its own screen rather than a panel on the board: a
meeting booked from a deal card was invisible the moment the card scrolled away. **SALES only** —
`39` §5 gives Marketing no booking action, so the entry would offer a button the server refuses.
It sits in the `Sales` group beside the funnel, because a meeting is worked before Handoff A.

**`NewDealForm` offers the intake custom fields and refuses the production ones.** The location
defines twenty; what the client is asking for (service, visa category, turnaround, language, page
count, their own description) is known at the point of sale, and Assigned Expert / Draft Link /
SLA Status / Docs Received / Actual Won Date are GHL's **shadow copy of facts EvalOS owns** — a
salesperson typing into them creates a second answer that goes stale the moment production moves.
Matched on **`fieldKey`**, never id (location-scoped) or label (renameable). A field the location
adds that is not on the list simply does not appear.

**`DealApplication` (2026-09-15) sits above `DealNotes` on an open deal card** — what the client
said, above what we said back. It **renders nothing** for a deal that did not come through the
portal, which is most of the board: the route answers 200 with a null payload for that case, so an
empty "no request" panel on every card would be noise on the common one. Every label it shows is
stored with the answer, because this app is a separate build from the client portal and cannot
import its catalog.
